...
var addIdFieldToSchema = exports.addIdFieldToSchema = function addIdFieldToSchema(schema) {
  var types = typesToAugment(schema);
...
var addOrderByEnumTypes = function addOrderByEnumTypes(schema) {
  ...
  var enumTypeSDL = types.reduce(function (acc, t) {
    return acc + ('\n      enum _' + t + 'Ordering {\n        ' + (0, _keys2.default)(schema.getTypeMap()[t]._fields).reduce(function (fieldStr, k) {
      return fieldStr + ('\n            ' + k + '_asc,\n            ' + k + '_desc,\n          ');
    }, '') + '\n      }\n    ');
  }, '');
...
var arrayQueryFieldsAndTypes = function arrayQueryFieldsAndTypes(schema) {
  ...
  var queryTypeNames = (0, _keys2.default)(schema._queryType._fields).filter(function (f) {
    return schema._queryType._fields[f].type.constructor === _graphql.GraphQLList;
  });
 ...
 * @param {*} schema
 */
var addOrderByToSchema = exports.addOrderByToSchema = function addOrderByToSchema(schema) {
...
function firstNonNullAndIdField(type) {
  var fields = (0, _keys2.default)(type.getFields()).filter(function (t) {
    return t !== '_id' && type.getFields()[t].type.constructor.name === 'GraphQLNonNull' && innerType(type.getFields()[t].type.name === 'ID');
  });
...
function firstNonNullField(type) {
  var fields = (0, _keys2.default)(type.getFields()).filter(function (t) {
    return t !== type.getFields()[t].type.constructor.name === 'GraphQLNonNull';
  });
